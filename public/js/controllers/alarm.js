...
angular.module('SmartAlarm')
  .controller('alarmCtrl', function ($scope, $http, $rootScope,$interval,$route, DB_queries,geolocation, fbLogin,$window) {
    ...
    $scope.calculateAlarm = function(){
        var ttg = new Date().setHours(0,0,0,0);
        var tta = $scope.parseTime($scope.todayAlarm.timeToArrive);
         console.log(tta);
        tta.setSeconds(tta.getSeconds()-$scope.durationValue);
        console.log(tta);

        //ttg.setHours($scope.todayAlarm.timeToArrive)
    }
